...
function Lk() {
    ...
    return (
        <>
            {vendorIsPopup && <VendorCreate state={setVendorIsPopup} vendor_name={user.username}/>}
            {createIsPopup && <AuctionCreate state={setCreateIsPopup}/>}
            <section>
                <div className="cont">
                    {user.vendor_photo_path !== null && <img className="big_ava" src={IMG_URL + user.vendor_photo_path}/>}
                    <h2>{user.username}</h2>
                    {user.vendor_photo_path !== null ?
                        <button onClick={() => setCreateIsPopup(true)} className="to_vendor">Выставить аукцион</button> :
                        <button onClick={() => setVendorIsPopup(true)} className="to_vendor">Стать продацом</button>
                    }
